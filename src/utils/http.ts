...
import { AxiosRequestConfig } from 'axios' // Đảm bảo import AxiosRequestConfig
    ...
    this.instance.interceptors.response.use(
      ...
      (error: AxiosError) => {
        ...
        if (isAxiosUnauthorizedError<ErrorResponse<{ name: string; message: string }>>(error)) {
          const config = (error.response?.config || {}) as AxiosRequestConfig // Ép kiểu rõ ràng Đảm bảo `config` không undefined.
          const url = config?.url || '' // Nếu không có `url`, gán giá trị mặc định là rỗng.
